<section class="py-5 text-center container">
    <form action="./index.php" method="post">
        <button class="btn-back" name="volver"><i class="bi bi-chevron-left"></i></button>
    </form>
    <div class="row pt-lg-5">
        <div class="col-lg-6 col-md-8 mx-auto">
            <h1 class="pro text-light">Proyectos</h1>
            <nav>
                <ul>
                    <li><a class="categorias" href="#" id="album-1-link"><i class="bi bi-bounding-box-circles"></i><span>Diseño gráfico</span></a></li>
                    <li><a class="categorias" href="#" id="album-2-link"><i class="bi bi-brush"></i><span>Ilustraciones</span></a></li>
                    <li><a class="categorias" href="#" id="album-3-link"><i class="bi bi-pc-display-horizontal"></i><span>Diseño web</span></a></li>
                </ul>
            </nav>
        </div>
    </div>
    <p class="intro text-center justify-text">Bienvenido a este portafolio digital donde podrás encontras
        mi pasión por el diseño y desarrollo web plasmada en cada uno de los proyectos que te presento.
        <br>Aqui encontraras una selección de ilustraciones, diseños y sitios web que he creado con mucha
        dedicación.
        <br>¡Espero que te gusten!
    </p>
</section>

<div id="album-container">
    <div id="album-1" class="album bg-light">
        <div class="container">
            <div class="row row-cols-1">
                <!-- Tamaño para las imagenes de portada 1294x300 -->
                <div class="col mt-3">
                    <div class="card shadow-sm my-3">
                        <svg id="primero" class="bd-placeholder-img card-img-top" width="100%" height="300px" role="img">
                            <title>Ilustraciones</title>
                            <!-- <rect width="100%" height="100%" fill="#55595c"></rect> -->
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/diseño.jpg" preserveAspectRatio="xMidYMid slice" />
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/rect1.png" class="negro" />
                            <text x="50%" y="50%" fill="white">"El mundo de la Tierra Media: Ilustraciones inspiradas en El Señor de los Anillos"</text>
                        </svg>
                    </div>
                </div>
                <div class="col">
                    <div class="card shadow-sm my-3">
                        <svg id="primero" class="bd-placeholder-img card-img-top" width="100%" height="300px" role="img">
                            <title>Ilustraciones</title>
                            <!-- <rect width="100%" height="100%" fill="#55595c"></rect> -->
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/diseño.jpg" preserveAspectRatio="xMidYMid slice" />
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/rect1.png" class="negro" />
                            <text x="50%" y="50%" fill="white">"El mundo de la Tierra Media: Ilustraciones inspiradas en El Señor de los Anillos"</text>
                        </svg>
                    </div>
                </div>
                <div class="col">
                    <div class="card shadow-sm my-3">
                        <svg id="primero" class="bd-placeholder-img card-img-top" width="100%" height="300px" role="img">
                            <title>Ilustraciones</title>
                            <!-- <rect width="100%" height="100%" fill="#55595c"></rect> -->
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/diseño.jpg" preserveAspectRatio="xMidYMid slice" />
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/rect1.png" class="negro" />
                            <text x="50%" y="50%" fill="white">"El mundo de la Tierra Media: Ilustraciones inspiradas en El Señor de los Anillos"</text>
                        </svg>
                    </div>
                </div>
                <div class="col mb-3">
                    <div class="card shadow-sm my-3">
                        <svg id="primero" class="bd-placeholder-img card-img-top" width="100%" height="300px" role="img">
                            <title>Ilustraciones</title>
                            <!-- <rect width="100%" height="100%" fill="#55595c"></rect> -->
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/diseño.jpg" preserveAspectRatio="xMidYMid slice" />
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/rect1.png" class="negro" />
                            <text x="50%" y="50%" fill="white">"El mundo de la Tierra Media: Ilustraciones inspiradas en El Señor de los Anillos"</text>
                        </svg>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div id="album-2" class="album bg-light">
        <div class="container">
            <div class="row row-cols-1">
                <!-- Tamaño para las imagenes de portada 1294x300 -->
                <div class="col mt-3">
                    <div class="card shadow-sm my-3">
                        <svg id="primero" class="bd-placeholder-img card-img-top" width="100%" height="300px" role="img">
                            <title>Ilustraciones</title>
                            <!-- <rect width="100%" height="100%" fill="#55595c"></rect> -->
                            <image x="0" y="0" width="100%" height="100%" xlink:href="#" preserveAspectRatio="xMidYMid slice" />
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/rect1.png" class="negro" />
                            <text x="50%" y="50%" fill="white"></text>
                        </svg>
                    </div>
                </div>
                <div class="col">
                    <div class="card shadow-sm my-3">
                        <svg id="primero" class="bd-placeholder-img card-img-top" width="100%" height="300px" role="img">
                            <title>Ilustraciones</title>
                            <!-- <rect width="100%" height="100%" fill="#55595c"></rect> -->
                            <image x="0" y="0" width="100%" height="100%" xlink:href="#" preserveAspectRatio="xMidYMid slice" />
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/rect1.png" class="negro" />
                            <text x="50%" y="50%" fill="white"></text>
                        </svg>
                    </div>
                </div>
                <div class="col">
                    <div class="card shadow-sm my-3">
                        <svg id="primero" class="bd-placeholder-img card-img-top" width="100%" height="300px" role="img">
                            <title>Ilustraciones</title>
                            <!-- <rect width="100%" height="100%" fill="#55595c"></rect> -->
                            <image x="0" y="0" width="100%" height="100%" xlink:href="#" preserveAspectRatio="xMidYMid slice" />
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/rect1.png" class="negro" />
                            <text x="50%" y="50%" fill="white"></text>
                        </svg>
                    </div>
                </div>
                <div class="col mb-3">
                    <div class="card shadow-sm my-3">
                        <svg id="primero" class="bd-placeholder-img card-img-top" width="100%" height="300px" role="img">
                            <title>Ilustraciones</title>
                            <!-- <rect width="100%" height="100%" fill="#55595c"></rect> -->
                            <image x="0" y="0" width="100%" height="100%" xlink:href="#" preserveAspectRatio="xMidYMid slice" />
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/rect1.png" class="negro" />
                            <text x="50%" y="50%" fill="white"></text>
                        </svg>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div id="album-3" class="album bg-light">
        <div class="container">
            <div class="row row-cols-1">
                <!-- Tamaño para las imagenes de portada 1294x300 -->
                <div class="col mt-3">
                    <div class="card shadow-sm my-3">
                        <svg id="primero" class="bd-placeholder-img card-img-top" width="100%" height="300px" role="img">
                            <title>Web</title>
                            <!-- <rect width="100%" height="100%" fill="#55595c"></rect> -->
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/diseño.jpg" preserveAspectRatio="xMidYMid slice" />
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/rect1.png" class="negro" />
                            <text x="50%" y="50%" fill="white">"Web El mundo de la Tierra Media: Ilustraciones inspiradas en El Señor de los Anillos"</text>
                        </svg>
                    </div>
                </div>
                <div class="col">
                    <div class="card shadow-sm my-3">
                        <svg id="primero" class="bd-placeholder-img card-img-top" width="100%" height="300px" role="img">
                            <title>Ilustraciones</title>
                            <!-- <rect width="100%" height="100%" fill="#55595c"></rect> -->
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/diseño.jpg" preserveAspectRatio="xMidYMid slice" />
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/rect1.png" class="negro" />
                            <text x="50%" y="50%" fill="white">"El mundo de la Tierra Media: Ilustraciones inspiradas en El Señor de los Anillos"</text>
                        </svg>
                    </div>
                </div>
                <div class="col">
                    <div class="card shadow-sm my-3">
                        <svg id="primero" class="bd-placeholder-img card-img-top" width="100%" height="300px" role="img">
                            <title>Ilustraciones</title>
                            <!-- <rect width="100%" height="100%" fill="#55595c"></rect> -->
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/diseño.jpg" preserveAspectRatio="xMidYMid slice" />
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/rect1.png" class="negro" />
                            <text x="50%" y="50%" fill="white">"El mundo de la Tierra Media: Ilustraciones inspiradas en El Señor de los Anillos"</text>
                        </svg>
                    </div>
                </div>
                <div class="col mb-3">
                    <div class="card shadow-sm my-3">
                        <svg id="primero" class="bd-placeholder-img card-img-top" width="100%" height="300px" role="img">
                            <title>Ilustraciones</title>
                            <!-- <rect width="100%" height="100%" fill="#55595c"></rect> -->
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/diseño.jpg" preserveAspectRatio="xMidYMid slice" />
                            <image x="0" y="0" width="100%" height="100%" xlink:href="./webroot/recursos/rect1.png" class="negro" />
                            <text x="50%" y="50%" fill="white">"El mundo de la Tierra Media: Ilustraciones inspiradas en El Señor de los Anillos"</text>
                        </svg>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>